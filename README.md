...
- **資料 (Material)** — ファイル添付または URL（YouTube 等の埋め込み対応）。書誌情報（著者・出典元・発行日・自由記述メモ）、信頼度（原本確認済/未確認）・権利状態、画像/YouTube のサムネイル。登録後はファイル/URL の差し替え不可（履歴性のため）。新規登録すると、その資料を引用するスタブ記事が自動で1本作られる。
- **トランスクリプト (Transcription)** — メディア資料（画像/動画/音声/PDF）に手動のトランスクリプト（文字起こし）を 1 件ずつ紐づけ、未着手 / 作業中 / 完了 の進捗を管理（`/transcriptions` ダッシュボード）。作成手法（手書き / AI / AI＋人手修正、AI の場合はサービス・モデル名）も記録できる。
