...
- **資料 (Material)** — ファイル添付または URL（YouTube / Dailymotion / Vimeo の埋め込み対応。タイトルと公開日も自動補完）。書誌情報（著者・出典元・発行日）、信頼度（原本確認済/未確認）・権利状態、画像/動画のサムネイル。登録後はファイル/URL の差し替え不可（履歴性のため）。新規登録時にチェックを入れると、その資料を引用するスタブ記事を1本自動作成できる（発行日は記事の年表日付に引き継がれる）。
